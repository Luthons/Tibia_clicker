/* Tibia Clicker - JavaScript Puro (revisado)
   Corrigido: erros de sintaxe (template literals), typos em expressões aritméticas,
   inicializações faltantes, funções que podiam acessar propriedades inexistentes,
   e algumas melhorias de robustez (checks null, valores padrão).
*/
...
(() => {
  ...
  function addToInventory(newItem) {
    ...
    if (existingIndex !== -1) {
      ...
      // if item has no uid (from pool), generate one
  ...
  const START = {
    ...
    attrs: {
      ...
      vit: 0 // adiciona 'vit' como 0 por padrão para compatibilidade
    },
    currentMana: 10,
  ...
  function newPlayerState(){
    // deep clone START to avoid accidental mutation of prototype START
  ...
  function load(){
    ...
    P = Object.assign(newPlayerState(), P); // garante campos faltantes
    ...
    P.attrs = Object.assign({maxMana:10, vit:0}, P.attrs || {});
    P.attrs.maxMana = P.attrs.maxMana || 10;
  ...
  function recomputeMaxHp(){
    const base = 100 + ((P.level || 1) - 1) * 10; // +10 HP por nível
    const attrHp = (P.attrs.vit || 0) * 5; // cada ponto em 'vit' = +5 HP
    const upgradeHp = (P.upgrades.maxHpLvl || 0) * 15; // upgrade da loja
    let gearHp = 0;
  ...
  function baseDamage(){
    ...
    // Crítico agora considera skill de archery e agilidade
    ...
    const amount = Math.floor(base * (isCrit ? critMult : 1));
  ...
  function startBossTimer(){
    ...
    bossTimer = setInterval(()=>{
      const t = now();
      if (t >= boss.nextHitAt){
        // boss ataca considerando defesa + shield skill
  ...
  function onPlayerDeath(){
    ...
    // recalcular nível a partir do totalXp
    ...
    P.xp = remain;
    P.xpToNext = xpForLevel(P.level);
    P.attrPoints = Math.max(0, P.attrPoints - 0);
    recomputeMaxHp();
    P.hp = P.attrs.maxHp;
  ...
  function modsToText(mods){
    ...
    return Object.entries(mods).map(([k,v])=>{
      ...
      const val = (k==='critdmg') ? `x${v.toFixed(2)}` : `${v}`;
      return `${label} ${val}`;
    }).join(', ');
  }
  ...
  function checkOfflineRewards(){
    ...
    if (elapsed > 6){
      const capped = Math.min(elapsed, 4 * 3600); // máximo 4h
      grantIdle(capped);
    }
  }
  ...
  function syncAll(){
    ...
    const xpBar = $('#xpBar'); if (xpBar) xpBar.style.width = ((P.xp / (P.xpToNext || 1)) * 100) + '%';
    ...
    if (manaBar) manaBar.style.width = clamp(((P.currentMana || 0) / (P.attrs.maxMana || 1)) * 100,0,100)+'%';
  ...
  function bindEvents(){
    ...
    // botões principais (checa existência)
  ...
  function init(){
    ...
    // Regeneração de mana a cada 3 segundos
